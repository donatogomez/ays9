...
class Action(object):
    ...
    @staticmethod
    def create(code, name, recurring, state):
        """
        :type code: str
        :type name: str
        :type recurring: ActionRecurring
        :type state: EnumActionState
        :rtype: Action
        """
    ...
    def __init__(self, json=None, **kwargs):
        ...
        val = data.get(property_name)
        if val is not None:
            datatypes = [ActionRecurring]
            try:
                self.recurring = client_support.val_factory(val, datatypes)
            except ValueError as err:
                raise ValueError(create_error.format(cls=class_name, prop=property_name, val=val, err=err))
        else:
            raise ValueError(required_error.format(cls=class_name, prop=property_name))
